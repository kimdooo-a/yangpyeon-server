...
## 🔄 컴퓨터 껐다 켤 때 재기동 절차 (시나리오 A 상세 SOP)

> 본 운영 호스트(`/home/smart/ypserver`)에서 **한 번이라도 정상 운영된 적이 있고**, `.env` 와 `dump.pm2`(PM2 영속 정보)가 보존된 상태를 전제합니다. 처음 셋업이라면 §🚀 빠른 시작 (시나리오 C) 으로 가세요.
>
> 이 §은 "Windows 종료 → 다시 켰음" 직후 사용자가 위에서 아래로 그대로 따라하면 되도록 작성된 SOP 입니다. 각 단계는 *예상 결과* 를 명시하니 비교하면서 진행하세요.

### 0. 사전 점검 — 1회만 (자동 복구 셋업 확인)

처음 한 번만 다음 명령을 WSL 안에서 실행하세요. 이미 셋업되어 있으면 그대로 두고 §1 부터 따라하면 됩니다.

```bash
# WSL Ubuntu 안에서

# 0-1. systemd 활성화 확인 (postgresql 자동 기동의 전제)
cat /etc/wsl.conf | grep systemd
# 기대: systemd=true  (없으면 추가하고 wsl --shutdown → 다시 진입)

# 0-2. PostgreSQL 부팅 자동 시작
sudo systemctl enable postgresql
sudo systemctl status postgresql   # active (running) 기대

# 0-3. PM2 부팅 자동 시작
pm2 startup systemd
# ↑ 출력된 sudo 명령을 그대로 1회 실행 (PM2 systemd unit 등록)
pm2 save
# ↑ 현재 프로세스 목록을 ~/.pm2/dump.pm2 에 저장 (재기동 시 이 목록으로 복원)
```

> **알려진 이슈 (S50 #2)**: Windows 재부팅 자동 복구 체인 전체 실증은 미완료. 본 절차는 *수동 베이스라인* — 자동화 셋업이 부분적이거나 실패해도 이 절차로 100% 복구 가능합니다.

---

### 1. Windows 부팅 + 로그인

별도 작업 없음. 일반적으로 사용하던 대로 Windows 로그인.

---

### 2. WSL 시작 (수동 트리거 필요)

> WSL2 는 Windows 부팅 시 자동 시작되지 않습니다. **명시적 트리거가 필요**합니다.

**Windows PowerShell 또는 Terminal** 에서:

```powershell
wsl --exec bash -lc 'echo WSL booted; uname -a'
```

기대 출력:
```
WSL booted
Linux <hostname> 5.15.x.x-microsoft-standard-WSL2 ...
```

또는 시작 메뉴에서 "Ubuntu" / "Windows Terminal" 의 WSL 프로필 클릭. WSL 이 시작되는 동안 systemd 가 `postgresql.service` 와 `pm2-smart.service` 를 자동 기동합니다 (§0 셋업이 되어 있다면).
...
**기대 결과** (3 프로세스 모두 `online`):

```
┌────┬──────────────────┬──────────┬─────────┬───────────┐
│ id │ name             │ uptime   │ ↺       │ status    │
├────┼──────────────────┼──────────┼─────────┼───────────┤
│ 1  │ cloudflared      │ <시간>   │ <복원수>│ online    │
│ 6  │ ypserver         │ <시간>   │ <복원수>│ online    │
│ 7  │ pm2-logrotate    │ <시간>   │ <복원수>│ online    │
└────┴──────────────────┴──────────┴─────────┴───────────┘
```

#### 3-A. 모두 `online` 인 경우 → §4 헬스체크로

#### 3-B. `stopped` 또는 `errored` 가 있는 경우 → 수동 복원

```bash
# 옵션 ①: dump.pm2 로부터 일괄 복원 (가장 빠름)
pm2 resurrect

# 옵션 ②: 개별 기동 (resurrect 실패 시)
pm2 start ypserver
...
#### 3-C. `pm2: command not found` 인 경우 → nvm 환경 미로드

```bash
# login shell + nvm 자동 source
bash -lic 'pm2 status'
# 또는
...
```bash
# 4-1. 로컬 루트 응답 — Next.js 도달 확인
curl -I http://127.0.0.1:3000/login
# 기대: HTTP/1.1 200 OK

# 4-2. SQLite 스키마 (ADR-021) — 4 테이블 무결 확인
cd /home/smart/ypserver
SQLITE_DB_PATH=$PWD/data/dashboard.db \
  node /mnt/e/00_develop/260406_luckystyle4u_server/scripts/verify-schema.cjs
# 기대: [verify-schema] OK — required tables present

# 4-3. PostgreSQL 응답
pg_isready -h localhost -p 5432
# 기대: localhost:5432 - accepting connections
```

#### 외부 접속 (브라우저)

```
https://stylelucky4u.com   → 로그인 페이지 도달
```

Cloudflare Tunnel 이 살아있고 ypserver 가 online 이면 즉시 응답. 502/503 이면 §5 트러블슈팅.
...
| 증상 | 원인 후보 | 조치 |
|---|---|---|
| `wsl --exec` 가 응답 없이 멈춤 | WSL VM 동결 | Windows 측 `wsl --shutdown` → 다시 시도 |
| `pm2: command not found` | nvm 환경 미로드 | `bash -lic 'pm2 ...'` (login shell) |
| `pm2 status` 빈 표 | PM2 데몬 첫 기동, dump.pm2 미로드 | `pm2 resurrect` |
| `ypserver` `stopped` | 사용자가 의도적으로 종료 | `pm2 start ypserver && pm2 save` |
| `ypserver` `errored` | DB 연결 / 환경변수 누락 | `pm2 logs ypserver --err --lines 50` 으로 원인 확인 |
| `localhost:3000` ECONNREFUSED | ypserver 미기동 | `pm2 start ypserver` |
| 외부 접속만 502/503 | cloudflared 만 down | `pm2 restart cloudflared` |
| `verify-schema` ELF mismatch | better-sqlite3 .node 가 Windows 바이너리 | `wsl-build-deploy.sh` 1회 재실행 (시나리오 B) |
| `SqliteError: no such table` | drizzle migration 미적용 | `wsl-build-deploy.sh` 재실행 (ADR-021 빌드 게이트 자동 적용) |
| `Error: P1001` Prisma | PostgreSQL 미기동 | `sudo systemctl start postgresql` |
| Cloudflare Tunnel 502 (cloudflared 도 online 인데) | tunnel 재협상 필요 | `pm2 restart cloudflared` |

---

### 6. 1줄 요약 (외워두면 편한 명령)

대부분의 케이스에서 다음 한 줄이면 충분합니다.

**Windows PowerShell 에서 WSL 트리거 + 상태 확인 + 필요 시 복원**:

```powershell
wsl --exec bash -lic 'pm2 resurrect; pm2 status; curl -sI http://127.0.0.1:3000/login | head -1'
```

기대 출력 끝줄: `HTTP/1.1 200 OK` 또는 `HTTP/1.1 307 Temporary Redirect`.
