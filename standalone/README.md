...
본 운영 호스트(/home/smart/ypserver)에서 한 번이라도 정상 운영된 적이 있고, .env 와 dump.pm2(PM2 영속 정보)가 보존된 상태를 전제합니다. 처음 셋업이라면 §🚀 빠른 시작 (시나리오 C) 으로 가세요.이 절차는 "Windows 종료 → 다시 켰음" 직후 사용자가 위에서 아래로 그대로 따라하면 되도록 작성되었습니다.


0. 사전 점검 — 1회만 (자동 복구 셋업 확인)

처음 한 번만 실행하세요. 이미 셋업되어 있으면 §1로 넘어갑니다.Bash# WSL Ubuntu 안에서

# 0-1. systemd 활성화 확인 (postgresql 자동 기동의 전제)
cat /etc/wsl.conf | grep systemd
# 기대: systemd=true (없으면 추가하고 wsl --shutdown → 다시 진입)

# 0-2. PostgreSQL 부팅 자동 시작 등록
...
sudo env PATH="$PATH:/home/smart/.nvm/versions/node/v24.14.1/bin" /home/smart/.nvm/versions/node/v24.14.1/lib/node_modules/pm2/bin/pm2 startup systemd -u smart --hp /home/smart

pm2 save
# ↑ 현재 프로세스 목록을 ~/.pm2/dump.pm2 에 저장

1. Windows 부팅 + 로그인
별도 작업 없음. 일반적으로 사용하던 대로 Windows 로그인.


2. WSL 시작 (수동 트리거)WSL2는 Windows 부팅 시 자동 시작되지 않으므로 명시적 트리거가 필요합니다.
...
기대 출력: WSL booted, Linux ...-microsoft-standard-WSL2


3. PM2 / 프로세스 자동 복원 확인
WSL 안에서 pm2 status를 입력하여 프로세스 상태를 확인합니다.
...
[상황별 조치]
3-A. 모두 online인 경우: 바로 §4 헬스체크로 이동.
3-B. stopped 또는 errored가 있는 경우:
Bash

pm2 resurrect   # 저장된 목록 불러오기
pm2 start ypserver # 개별 기동 필요 시
pm2 save        # 상태 확정


3-C. pm2: command not found 발생 시:
Bash
bash -lic 'pm2 status' # Login Shell로 환경변수 로드



4. 헬스체크 (4 단계)

4-1. 로컬 응답 확인Bashcurl -I http://127.0.0.1:3000/login
# 기대: HTTP/1.1 200 OK

4-2. SQLite 스키마 무결 확인 (중요: 환경 격리)
Windows 경로와의 충돌을 피하기 위해 반드시 아래 순서로 실행합니다.

Bash

cd /home/smart/ypserver

# 1. 스크립트 파일이 없다면 복사 (최초 1회)
mkdir -p scripts
cp /mnt/e/00_develop/260406_luckystyle4u_server/scripts/verify-schema.cjs ./scripts/

# 2. 무결성 검증 실행
SQLITE_DB_PATH=$PWD/data/dashboard.db node ./scripts/verify-schema.cjs
# 기대: [verify-schema] OK — required tables present


4-3. PostgreSQL 응답 확인
...
4-4. 외부 접속 확인
브라우저에서 https://stylelucky4u.com 접속하여 로그인 페이지 도달 확인.
...
증상					원인 후보					조치
외부 접속 502/503		cloudflared 정지/오류			pm2 restart cloudflared
verify-schema ELF 		에러Windows용 바이너리 혼용	rm -rf node_modules && npm install (WSL 내부에서)
verify-schema 파일 없음	경로 설정 오류				§4-2의 cp 명령어로 파일 복사 재수행
Prisma / DB 연결 에러		PostgreSQL 미기동			sudo systemctl restart postgresql
메모리 부족/멈춤			WSL 자원 할당 초과			Windows에서 wsl --shutdown 후 재시작



6. 요약 명령 (Quick Restore)
대부분의 상황에서 Windows PowerShell에 아래 한 줄만 복사해 넣으면 복구와 점검이 동시에 끝납니다.

PowerShell
wsl --exec bash -lic 'pm2 resurrect; pm2 status; cd ~/ypserver; SQLITE_DB_PATH=$PWD/data/dashboard.db node ./scripts/verify-schema.cjs; curl -sI http://127.0.0.1:3000/login | head -1'

최종 출력에 [verify-schema] OK와 HTTP/1.1 200 OK가 보이면 운영 준비 완료입니다.
