...
- **세션 22 (Phase 14b E2E 재수행 + `@updatedAt` 버그)**: `docs/solutions/2026-04-17-phase-14b-updated-at-no-db-default.md` 참조. Prisma `@updatedAt` 필드는 DB DEFAULT를 생성하지 않아 Phase 14b raw SQL INSERT가 NOT NULL 위반 500. 현재 `RowFormModal` 3상태 "keep" 기본값으로 UI "행 추가" 실사용자 경로가 깨져 있음. Phase 14c에서 `@default(now()) @updatedAt` 병기 마이그레이션 권장(9개 모델 대상).
