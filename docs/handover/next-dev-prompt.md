...
docs/handover/260425-session55-ypserver-skill-v2-deploy.md (ypserver 글로벌 스킬 v1→v2 전면 리팩터: ~/dashboard→~/ypserver, Phase 1 Windows next build 삭제, wsl-build-deploy.sh 단일 위임 / CK-39)
docs/solutions/2026-04-25-skill-ops-drift-pattern.md  CK (39) — 글로벌 스킬 vs 운영 진화 분기 패턴 / 사용자 직감 트리거 / 갱신-우선 배포 순서 / drift 진단 절차
docs/handover/260425-session54-audit-silent-failure-diagnostic.md (cleanup-scheduler.ts 두 catch 블록 진단 패치 + CK-38) — S56 본문이 ROI 실증 (5일 연속 audit failed → 12:52:52 SqliteError 표면화)
docs/solutions/2026-04-25-cleanup-scheduler-audit-silent-failure.md  CK (38) — `catch {}` vs `catch (err) + structured log` 진단성 1bit vs 다차원 비대칭 / 검증 비용 비대칭 ROI
...
### 세션 56 신규 (2026-04-25) — ADR-021 Audit fail-soft + 마이그레이션 self-heal

- **로그인 장애 근본 해결 4단계 적용** — 운영 DB 빈 SQLite (`audit_logs` 부재) 로 인한 로그인 500 에러를 4층 결함 (L1 전술 / L2 빌드 / L3 강결합 / L4 sweep 누락) 모두 해결.
  - **Step 1**: 운영 DB(`/home/smart/ypserver/data/dashboard.db`) 에 drizzle 마이그레이션 직접 적용 → `audit_logs, ip_whitelist, metrics_history, __drizzle_migrations` 4개 테이블 생성.
  - **Step 2**: `src/lib/audit-log-db.ts` 에 `safeAudit(entry, context?)` 추가 → 11개 도메인 콜사이트 (`login-finalizer`, 9개 v1/admin 라우트, cleanup-scheduler) 일괄 sweep. `writeAuditLogDb` 는 `@internal` 마킹.
  - **Step 3**: 빌드 게이트 + 운영 self-heal 이중 안전망. `scripts/run-migrations.cjs` + `scripts/verify-schema.cjs` 신규. `wsl-build-deploy.sh` [6/8] migrate / [7/8] verify 단계 추가 (verify 실패 시 PM2 reload 차단). `pack-standalone.sh` 가 drizzle migrations → `<bundle>/db-migrations/` 동봉. `instrumentation.ts` 가 부팅 시 `applyPendingMigrations()` 호출.
  - **Step 4**: `docs/research/decisions/ADR-021-audit-cross-cutting-fail-soft.md` 정식화 (cross-cutting fail-soft invariant) + CK +1 (`docs/solutions/2026-04-25-audit-fail-soft-and-migration-self-heal.md`).
- **검증**: `npx tsc --noEmit` 0 errors / `vitest cleanup-scheduler.test.ts` 13/13 PASS / 운영 `verify-schema.cjs` OK / 멱등성 확인(재실행 시 skip).
- **세션 54 ELF 가설 최종 부정** — silent failure 정체는 ELF mismatch 도 SQLite 락 도 아닌 단순 **마이그레이션 미적용 빈 DB**. 12:52:52 `SqliteError: no such table` 가 결정적 증거. ELF 가설은 별개 트랙으로 잔존(2026-04-19~21 ERR_DLOPEN_FAILED 4건) — WSL 재배포로 흡수된 듯하나 고정 검증은 다음 standalone 재배포 시.
- **빌드 + 배포 미실행** — 본 세션 변경은 운영 코드 반영 안 된 상태. 다음 작업: `bash /mnt/e/00_develop/260406_luckystyle4u_server/scripts/wsl-build-deploy.sh` 1회 실행 → 새 [6/8] [7/8] 단계가 자체 검증되는지 확인. 운영 DB 는 이미 Step 1 으로 복구되어 있어 migrate 는 idempotent skip 예상.
- ~~**알려진 잔류 위험 (낮음)**: `safeAudit` 실패 시 warn 로그만 — audit 누락이 모니터링 없이 누적될 수 있음. 별도 트랙으로 audit-failure 카운터 메트릭 권장 (ADR-021 §결과·부정).~~ — **세션 56 §보완 완료** (2026-04-25). `src/lib/audit-metrics.ts` + `GET /api/admin/audit/health` 추가, ADR-021 §amendment-1 정식화. PM2 reload 시 리셋되는 in-process 카운터 (1차 가시성 — 누적 추세는 audit_logs 가 source of truth). 9/9 vitest PASS.
...
- **블로킹 항목 5건 (진행 불가 사유 명시)**:
  - **C — Postgres `~/ypserver/.env` 패스워드 rotation (<DB_PASSWORD> 평문)**: 파괴적 (DATABASE_URL 동기화 필수, 1초 다운타임 가능). 명시적 승인 필요.
