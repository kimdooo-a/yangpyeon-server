...
// 출처: Almanac scripts/ingest/gemini.ts 포팅
//       + 시스템 프롬프트에 subcategorySlug 필드 추가
//       + AGGREGATOR_LLM_DAILY_BUDGET 환경변수 기반 자정 리셋 카운터
// 정책:
//   - 무료 티어 10 RPM 보호: 호출 간 6.5초 간격
//   - 일일 한도 초과 → 호출 스킵하고 규칙 결과만 사용 (graceful degradation)
//   - 응답 파싱 실패도 무시 — 파이프라인은 계속
// =============================================================================
...
export async function enrichItem(item: RawItem): Promise<EnrichedItem> {
  ...
  const c = getClient();
  if (!c) return baseEnriched; // API 키 없음 → 규칙만
  ...
  try {
    ...
    const aiSummary = typeof parsed.summary === "string" ? parsed.summary.slice(0, 2000) : undefined;
    const aiTags = Array.isArray(parsed.tags)
      ? parsed.tags.slice(0, 8).map((t) => String(t)).filter(Boolean)
      : undefined;
    const aiLanguage = typeof parsed.language === "string" ? parsed.language.slice(0, 10) : undefined;
    ...
    // 한도 차감은 이미 발생 가능 — 다음 호출은 throttle 만 적용
