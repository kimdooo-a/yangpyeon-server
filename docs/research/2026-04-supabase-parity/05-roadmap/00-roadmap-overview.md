...
| 항목 | 값 | 근거 |
|------|-----|------|
| 총 기간 | **50주 (초판) / 25~54주 (2026-04-25 재보정 범위)** | §8.1.1 시나리오 A·B 병기 |
| 총 공수 | **~870h (초판) / ~916h (계획외 +46h 흡수, 2026-04-25)** | `_CHECKPOINT_KDYWAVE.md Wave 4` + §8.1.1 |
...
### 8.1 1인 운영 버퍼 정책 (20% → 25%로 상향, 2026-04-25 재보정)
...
#### 8.1.1 2026-04-25 재보정 — 계획외 공수 + 실측 속도 반영

**계획외 공수 발견 (Wave 4 미반영)**:

| 항목 | 출처 | 추가 공수 |
|------|------|---------|
| TIMESTAMPTZ 마이그레이션 캐스케이드 | 세션 40~44 (5세션) | +20h (Date 직렬화 감사 4파일 8곳 raw SQL 전환 + 0-row 테이블 17파일 선제 수정 + INSERT timestamptz 시프트 검증) |
| HS256 legacy 제거 | 세션 45 | +6h |
| Phase 15-D 보강(refresh rotation·revoked·touch throttle) | 세션 36~39 | +12h (Wave 4 청사진 22h 외 추가) |
| 세션 50 standalone 패키지 (ADR-020 신설) | 세션 50 | +8h |
| **소계** | — | **+46h** |

**실측 속도 vs 계획**:

| 차원 | 계획(§8.1 초판) | 실측 (2026-04-19~25 1주) | 비고 |
|------|---------------|-----------------------|------|
| 주당 코딩 가능 시간 | 20h/주 | ~50h+/주 (체감) | Opus 4.7 에이전트 병렬 환경 + 1인 집중 |
| Phase 15 소요 | 4주 | ~3-4일 | -85% 단축 (세션 32~39 압축) |
| Phase 16a Vault | 16a 부분 ~1주 | ~1일 (세션 48) | 압축 |

**보정안 (두 시나리오 병기)**:

```
시나리오 A — 버퍼 25% 상향 + 계획외 +46h 흡수:
  순수 공수 (870 + 46)h × 1.25 / 20h/주 = 57.3주
  → 50주 안에 GA는 어려움, 53~54주 권장

시나리오 B — 실측 속도 반영(주당 50h):
  순수 공수 (870 + 46)h / 50h/주 = 18.3주 (이론적 최소)
  실제 버퍼 포함 (916h × 1.25) / 50h/주 = 22.9주
  → 25주 이내 GA 가능 (계획의 50% 단축)
```

**현실 위치 추정**: 시나리오 A와 B 사이 중간값. 본 1주 압축 사례는 (1) Phase 15가 본인이 가장 익숙한 인증 도메인 + (2) Wave 청사진의 WBS 12 태스크가 잘 분해되어 있어 단순 실행 가능했던 케이스. Phase 18 SQL Editor 320h(40일)는 Monaco/AI/Plan Visualizer 미지 영역이 많아 계획 속도에 가까울 가능성. **Phase 16 종료 시점에 실제 누적 공수 vs 계획 비율을 측정하여 Phase 17 시점에 50주 vs 25주 사이 어느 위치인지 확정 권장.**

**측정 지표 추가 (Phase 16 종료 시 수집)**:
- 누적 코딩 시간 (`git log --since=Phase15-start --shortstat | grep insertions` × 평균 시간/100lines)
- 누적 세션 수 (handover 파일 수)
- 계획외 공수 비율 (Wave 4 청사진 미반영 작업 / 전체 작업)

→ §13 로드맵 위험 신호 표 "Phase 공수 초과 +30%" 임계값에 **하향 이탈(계획 대비 50% 미만 소요)도 동등 가중치 측정 항목으로 추가**. 빠른 진행이 곧 품질 확보를 의미하지 않으므로 (예: 세션 50 standalone 인플레이션 700패키지) 동일 비중 모니터링 필요.
...
| **Phase 공수 하향 이탈** (2026-04-25 신설) | 계획 대비 -50% (1주 압축 등) | 해당 Phase 종료 시 | 품질 회귀 점검(예: standalone 700패키지 인플레이션 사례). 빠른 진행 ≠ 품질 보장 |
...
| **계획외 작업 비율** (2026-04-25 신설) | Wave 4 청사진 외 작업이 누적 공수의 15% 초과 | Phase 17 종료 시 측정 | Phase 18 착수 전 Wave 4 ADR 재검토 (예: ADR-019 argon2id, ADR-020 standalone 같은 신규 ADR로 흡수) |
...
- **매 Phase 완료 시 git 태그 생성** (예: `v0.1.0-alpha.1` Phase 15 완료 — 2026-04-25 보정. 세션 39 시점 SHA `9d2a61e`에 소급 부여 완료)
- **공수 자동 측정 (2026-04-25 신설)**: Phase 종료 시 `git log --since=<phase-start> --until=<phase-end> --shortstat` 통계와 handover 파일 수를 §8.1.1 시나리오 A vs B 위치 추정에 사용
