...
| ADR-019 | Password Hash argon2id 전환 (bcrypt → @node-rs/argon2, SP-011) | Accepted | 17 |
| ADR-020 | 배포 경로 — Next.js standalone + rsync + pm2 reload (ADR-015 재검토 트리거 발동, 세션 50) | Accepted (Supersedes ADR-015 부분) | 16 |
| ADR-021 | 감사 로그 cross-cutting fail-soft + 마이그레이션 self-heal (세션 56) | Accepted | 16 |
...
- **세션 30 (2026-04-19) 보완 — 3건**:
  1. **사실관계 정정**: 본 ADR 컨텍스트의 "bcrypt" 언급은 프로젝트 실제 현행을 반영. `package.json` 확인 결과 **`bcrypt@^6.0.0`** (N-API native) 사용 중. 일부 하위 문서에서 "bcryptjs"로 기재된 것은 오기 — `bcrypt`로 정정 필요.
  2. **argon2id 전환 예정**: SP-011 실측 기반 `@node-rs/argon2` 도입 확정. bcrypt 대비 13× faster (bcrypt(12) 172ms vs argon2id 19.8ms p95). Phase 17 Auth Core 완성 시점에 점진 마이그레이션 실행. 상세는 **ADR-019** (argon2id 전환, 본 문서 §4.1 참조).
  3. **Session 인덱스 설계 확정**: SP-015 실측 기반 `(userId, expiresAt)` 복합 인덱스 + cleanup job (일 1회) 채택. PG partial index with `NOW()`는 IMMUTABLE 제약으로 불가. 상세는 `docs/solutions/2026-04-19-pg-partial-index-now-incompatibility.md`.
...
- **세션 30 (2026-04-19) 보완 — SP-016 축약 Pending**:
  - 본 세션 물리 측정 미수행(weed CLI 미설치 + 50GB 디스크/시간 필요). 설계 체크리스트 + 실측 절차 + proto-verdict "조건부 Go" 문서화만 완료
  - **추가 proto-권고**: filer 저장 엔진을 기본 sqlite 대신 **leveldb** 로 설정 권장 (50만+ 엔트리 대비). 상세는 `docs/research/spikes/spike-016-seaweedfs-50gb-result.md` §5 체크리스트
  - 실측 완료 시 ASM-4 검증 결과 + 실제 메모리/처리량 수치를 `07-storage-blueprint.md` §운영 가이드에 반영 필요
...
- **재검토 트리거**:
  1. ~~isolated-vm v6 Node 24 ABI 호환 깨짐 (ASM-5 EWI)~~ — **RESOLVED 2026-04-19 (SP-012)**. `isolated-vm@6.1.2` + Node v24.14.1 WSL2 실증 완료: 기본 실행 OK, cold start p95 0.909ms(목표 50ms의 55× 여유), 메모리 격리 + 누수 환산 0.09MB/10분. 상세: `docs/research/spikes/spike-012-isolated-vm-v6-result.md`, Compound Knowledge `docs/solutions/2026-04-19-isolated-vm-v6-node24-wsl2-verified.md`.
  2. Deno 2.x 에서 Next.js 통합 공식 지원 시 L2 단독 고려
  3. Edge function invocation 월 > 10만 → Vercel Sandbox 비용 재평가

- **세션 30 (2026-04-19) 보완**:
  - cold start 목표 상향: spec의 p95 ≤ 50ms → 실측 기준 **p95 < 2ms** (안전 계수 2× 적용)
  - Phase 19 구현 패턴 확정 — `isolate-pool.ts` 코드 템플릿은 [spike-012 §8](../../../research/spikes/spike-012-isolated-vm-v6-result.md#8-운영-체크리스트-phase-19-착수-시) 참조
...
- **세션 30 (2026-04-19) 보완 — SP-013 축약 Pending**:
  - 본 세션 물리 측정 미수행(30분 DML 주입 + 슬롯 손상 recovery + `@supabase/realtime-js` presence_diff 구조 검증 필요)
  - **proto-권고**: `max_slot_wal_keep_size = 2GB` GUC 필수 설정 (Consumer 다운 시 WAL 무제한 적체 방지). Consumer 감시 cron 1분 주기 (WAL lag 50% 도달 시 알림)
  - 실측 완료 시 DQ-RT-3/RT-5 답변 확정 + `11-realtime-blueprint.md` §슬롯 운용 가이드에 WAL lag 쿼리 추가 필요
  - 상세 체크리스트: `docs/research/spikes/spike-013-wal2json-slot-result.md` §5
...
- **세션 30 (2026-04-19) 보완 — JWKS 3분 캐시 검증 (SP-014)**:
  - jose `createRemoteJWKSet({ cacheMaxAge: 180_000 })`: 로컬 검증 p95 **0.189ms** · hit rate 99.0% · Cloudflare Tunnel RTT p95 148.7ms · 실효 지연(hit 99% 반영) **약 1.62ms**
  - **중요 명료화**: "3분 grace"는 jose 클라이언트 캐시만으로 성립하지 않는다. **JWKS 엔드포인트가 구·신 키를 동시에 서빙**해야 grace 보장. `/api/.well-known/jwks.json`이 `SigningKey.retireAt > NOW()` 조건으로 retire 대기 키도 응답에 포함하도록 운용 — 상세는 `03-auth-advanced-blueprint.md` §JWKS와 Compound Knowledge `docs/solutions/2026-04-19-jwks-grace-endpoint-vs-client-cache.md`
  - **DQ-12.4 Resolved**: Cloudflare Workers 앞단 캐시 **현 시점 불필요**. 재검토 트리거 2건: (a) Tunnel 530 재발률 > 1%/일 → hit rate 95% 하회, (b) JWT 검증량 > 1000 RPS → miss 절대 빈도 10/s 초과
...
- **세션 30 (2026-04-19) 보완 — PM2 cluster:4 조건부 전환 명시 (SP-010)**:
  - SP-010 실측: cluster:4 vs fork RPS **+39.9%** (Node `cluster` 모듈, 내부적으로 PM2 cluster:4와 동등), SQLITE_BUSY **0.000%** (WAL + busy_timeout=5000ms)
  - **즉시 cluster:4 전환 불필요**. Phase 16 Operations 착수 시점에 다음 조건 중 1건 이상 충족 시 전환:
    1. p95 응답 지연 > 200ms 지속 (5분 이상)
    2. CPU 사용률 > 70% 지속 (5분 이상)
    3. `cloudflared` 503 에러율 > 0.1%
  - **전환 시 체크리스트**: (a) node-cron 모든 잡에 PostgreSQL `pg_try_advisory_lock` 래퍼 적용, (b) SQLite 접근부에 `busy_timeout=5000ms` 통일, (c) `process.on("SIGTERM", ...)` graceful shutdown 구현, (d) `pm2 reload` 무중단 검증
  - **치명적 운영 규칙 (신규)**: `pm2 delete all`/`pm2 restart all`/`pm2 stop all` 명령 절대 금지 — PM2 v6.0.14에서 `--namespace` 필터가 무시되어 프로덕션 프로세스까지 삭제되는 버그 실증 (SP-010 실험 중 `dashboard`+`cloudflared` 삭제 사고 발생, `pm2 resurrect`로 복구). `/ypserver` 스킬 및 CI 스크립트에 safeguard 추가. 상세는 `docs/solutions/2026-04-19-pm2-delete-all-namespace-bug.md`

- **세션 50 (2026-04-19) 보완 — 배포 경로 부분 대체 (ADR-020 신설)**:
  - **상태 변경**: 본 ADR-015의 *Capistrano-style symlink + releases/* 디렉토리 구조는 **세션 49 bootstrap 스크립트 작성 단계까지 진행**되었으나, 세션 50에서 동일 무중단 목표를 더 단순한 경로(`Next.js standalone 패키지 + rsync 동기화 + pm2 reload`)로 달성 가능함이 실증됨.
  - **결과**: 본 ADR-015의 *PM2 cluster:4 + canary 서브도메인* 부분은 **유효 유지**, *Capistrano-style symlink/releases* 부분은 **ADR-020에 의해 부분 대체(Superseded)**.
  - **재검토 트리거 추가 (Capistrano 재진입 조건)**:
    1. 무중단 핫픽스 중 rsync 부분 실패로 인한 미정합 산출물 발생 (1회 이상)
    2. `pm2 reload` 실패 시 자동 롤백 자산 부재로 다운타임 30초+ 1회 이상
    3. 사용자 2명+ 동시 운영으로 atomic 롤백 자산이 사실상 필수가 됨
    4. canary.stylelucky4u.com 분기 시점에 두 환경 동시 보존 필요
  - **근거 문서**: `docs/handover/260419-session50-standalone-package.md` 토픽 1·8·10, `docs/solutions/2026-04-19-nextjs-standalone-output-misunderstanding.md`
  - **연계**: ADR-020 (배포 경로) 본문 참조
...
- **결정자**: 세션 30 (본 세션)
- **초기 제안 번호**: ADR-022 (세션 30 초반 문서에서 사용) → 본 문서 §3.1 규칙에 따라 **ADR-019** 로 정식 등록. 하위 참조의 ADR-022 언급은 모두 ADR-019와 동일 결정을 가리킴.

- **컨텍스트**:
  - 프로젝트 현행: `bcrypt@^6.0.0` (N-API native). ADR-006 본문의 "bcryptjs" 표기는 오기로 판명.
  - Wave 5 DQ-AC-1: "bcryptjs → @node-rs/argon2 교체 시점? 성능 5×, native 모듈 부담" — Pending 상태였음.
  - NFR-SEC.10 패스워드 해시 알고리즘 강도 + NIST SP 800-63B 권장 KDF 준수 요구.

- **결정**:
  **Phase 17 Auth Core 완성 시점에 `@node-rs/argon2` 도입 + 점진 마이그레이션**.

  핵심:
  1. `@node-rs/argon2` 의존성 추가 (prebuilt binary, WSL2 설치 3.3초)
  2. `src/lib/auth/password.ts` 신규 — `verifyPassword(user, password)` 함수가 해시 접두사(`$2*` vs `$argon2id$`)로 알고리즘 자동 판별
  3. bcrypt 검증 성공 시 argon2id로 **즉시 재해시 저장** (DB UPDATE, fire-and-forget 가능)
  4. 신규 가입자는 즉시 argon2id
  5. **User 스키마 변경 불필요** (`passwordHash` 필드 유지)
  6. `bcrypt` 의존성은 90일 유지 후 제거 (로그인 tracking으로 잔여 bcrypt 해시 0 확인 후)

- **고려한 대안**:
  - **bcrypt cost factor 상향 (12 → 14)**: 보안 보강은 되지만 속도 저하 추가. argon2 대비 메모리-하드성 부재 — **부분 채용 불가**.
  - **현행 bcrypt 유지**: CON-10 native 모듈 부담 우려는 N-API prebuilt로 사실상 해소(`docs/solutions/2026-04-19-napi-prebuilt-native-modules.md`) — **거부**.
  - **scrypt**: argon2id 대비 FPGA 저항성 낮음. NIST 권장 순위도 뒤 — **거부**.

- **결과**:
  - **긍정**:
    - SP-011 실측 **13× faster** (bcrypt(12) 172.2ms p95 vs argon2id(default) 19.8ms p95)
    - 1000 사용자 점진 마이그레이션 오류 **0**
    - NIST SP 800-63B 권장 KDF 정합
    - 로그인 지연 감소로 체감 UX 개선
  - **부정**:
    - `@node-rs/argon2` 의존성 추가 (native). N-API prebuilt로 node-gyp 부담 없음
    - Phase 17 구현 시 `verifyPassword` 함수 래핑 공수 약 1h

- **근거 문서**:
  - `docs/research/spikes/spike-011-argon2-result.md` (전체)
  - `docs/solutions/2026-04-19-napi-prebuilt-native-modules.md`
  - ADR-006 §세션 30 보완

- **재검토 트리거**:
  1. OWASP 권장 argon2 파라미터 변경 (memoryCost/timeCost 상향 권고 시 재튜닝)
  2. N-API 버전 호환성 깨짐 (Node 메이저 업그레이드 25+ 시점)
  3. argon2id 취약점 발견 (이론적, 현재까지 없음)

---

### ADR-020: 배포 경로 — Next.js standalone + rsync + pm2 reload (ADR-015 Capistrano 부분 대체)

- **상태**: Accepted (ADR-015의 Capistrano-style symlink/releases 부분을 Superseded)
- **날짜**: 2026-04-19 (세션 50)
- **결정자**: 세션 50 (1인 운영자 + Claude Opus 4.7)

- **컨텍스트**:
  ADR-015는 `releases/{timestamp} + current symlink + 5초 롤백`의 Capistrano 패턴을 채택. 세션 49에서 `scripts/capistrano-bootstrap.sh` + `scripts/phase16-bootstrap-verify.sh` 작성으로 bootstrap 단계 진입. 세션 50에서 사용자가 빈 `standalone/` 디렉토리를 생성한 신호를 발견하여 Next.js 16 `output: 'standalone'` 모드를 재도입(세션 3 결정 역전). 빌드 산출물 패키징 자동화(`scripts/pack-standalone.sh`) + WSL 배포 5단계 + PM2 fork 모드 ecosystem이 완성되어 **무중단 ~1초 reload + rsync `--delete` 증분 동기화**로 ADR-015의 핵심 목표(다운타임 0 + 빠른 롤백)를 달성 가능함이 실증됨.

- **결정**:
  **Phase 16 운영의 기본 배포 경로는 Capistrano symlink가 아닌 `standalone 패키지 + rsync + pm2 reload`로 한다.** ADR-015의 *PM2 cluster:4* 와 *canary 서브도메인 시간차 롤아웃* 결정은 그대로 유효. ADR-015의 *releases/{timestamp} + current symlink + 자동 release 보존* 메커니즘은 본 ADR이 부분 대체.

  핵심 산출물 (이미 git 추적 중):
  - `next.config.ts` `output: 'standalone'` (세션 3 결정 역전)
  - `scripts/pack-standalone.sh` (NFT + 수동 보강 + 시크릿 방어 5단계)
  - `standalone/{start.sh, ecosystem.config.cjs, install-native-linux.sh, .env.production.example, README.md}` 5개 화이트리스트 헬퍼
  - `.gitignore` `/standalone/` 제외 + 헬퍼 5개 화이트리스트
  - `docs/solutions/2026-04-19-nextjs-standalone-output-misunderstanding.md` (세션 3 오해 해부 + 운용법)

  롤백 자산:
  - 이전 패키지 백업 1세트 보존 (`~/ypserver.previous`) — Capistrano 5초 롤백 대비 ~3-5초 (백업 디렉토리 swap + pm2 reload)

- **고려한 대안**:
  - **Alt-1 (ADR-015 Capistrano 그대로 진행)**: 세션 49 bootstrap까지 작성 완료 — 운영 단순성·구현 공수 측면에서 standalone 방식이 우위로 판단되어 진입 보류. **거부**(세션 50 단계). 단, ADR-015 본 보완에서 정의한 4 트리거 충족 시 재진입.
  - **Alt-2 (Docker Compose)**: ADR-015에서 이미 거부. 본 결정에서도 동일.
  - **Alt-3 (`npm ci --production` 타겟 빌드)**: standalone NFT 설계 철학 위배 — 단, 본 ADR §주의사항에서 `install-native-linux.sh` fallback 시 일부 발생함을 인지.

- **결과**:
  - **긍정**:
    - 운영 단순성 — Capistrano releases 디렉토리 관리 부담 0
    - 무중단 핫픽스 ~1초 (`rsync --delete + pm2 reload`)
    - HTTP 200 OK + Ready in 0ms 실측(세션 50, BUILD_ID `WYBvMpH13rjEvlzoA_mm1`)
    - WSL ext4 홈 디렉토리(`~/ypserver`)로 NTFS 오버헤드 회피
    - sudo 최소화(개인 프로젝트 운영 규모 적합)
  - **부정**:
    - **standalone 패키지 인플레이션**: `install-native-linux.sh` fallback이 700+ 패키지를 끌어와 131MB → ~400MB 팽창 (개선 대상, P0)
    - **롤백 자산 메커니즘이 명시적 백업 1세트에 의존** (Capistrano releases 누적 N개 대비 단순)
    - **Windows 재부팅 자동 복구 미실증** (P0)
    - **PostgreSQL systemd enable 미확인** (P1)

- **근거 문서**:
  - `docs/handover/260419-session50-standalone-package.md` 전체 (특히 토픽 1, 3, 7, 8, 9, 10)
  - `docs/solutions/2026-04-19-nextjs-standalone-output-misunderstanding.md`
  - `scripts/pack-standalone.sh`, `next.config.ts`, `standalone/README.md`

- **재검토 트리거 (Capistrano 재진입 조건 — ADR-015 보완본과 동일)**:
  1. 무중단 핫픽스 중 rsync 부분 실패로 인한 미정합 산출물 발생 (1회 이상)
  2. `pm2 reload` 실패 시 자동 롤백 자산 부재로 다운타임 30초+ 1회 이상
  3. 사용자 2명+ 동시 운영으로 atomic 롤백 자산이 사실상 필수가 됨
  4. canary.stylelucky4u.com 분기 시점에 두 환경 동시 보존 필요

- **즉시 후속 조치 (Phase 16b 잔여 P0)**:
  - 이슈 #1: `install-native-linux.sh` 패키지 인플레이션 개선 (옵션 A: prebuild 우선 명시 버전 / 옵션 B: pack 시점 native 바이너리 번들)
  - 이슈 #2: Windows 재부팅 1회 리허설로 WSL → PM2 → cloudflared 자동 복구 체인 실증
  - 이슈 #3: `pm2 install pm2-logrotate` (P1)
  - 이슈 #4: `sudo systemctl enable postgresql` (P1)
...
- **상태**: Accepted
- **날짜**: 2026-04-25 (세션 56)
- **결정자**: 프로젝트 오너 (1인 운영) + Claude Opus 4.7
- **본문 정식 위치**: [`docs/research/decisions/ADR-021-audit-cross-cutting-fail-soft.md`](../../decisions/ADR-021-audit-cross-cutting-fail-soft.md) — 본 §1 항목은 Wave registry 컨텍스트 요약. 9개 필드 전체와 §5 재검토 트리거는 정식 파일 참조.

- **컨텍스트**:
  세션 56 시작 시 사용자 로그인이 "서버 연결 오류"로 차단됨. PM2 에러 로그 `2026-04-25 12:52:52 SqliteError: no such table: audit_logs` 가 인과 사슬 직접 증거. 4층 결함 — L1 운영 DB 빈 파일(drizzle 마이그레이션 미적용) / L2 빌드 파이프라인에 마이그레이션·검증 게이트 부재 / L3 도메인 임계 경로(login)가 동기 audit throw 에 fail-tied / L4 세션 54 silent-failure 진단 패턴이 cleanup-scheduler 1곳에만 적용되고 11개 도메인 콜사이트 누락. 세션 50 standalone 도입 후 `pack-standalone.sh` 가 prisma 마이그레이션만 동봉하고 drizzle 누락한 비대칭이 운영 환경에 빈 DB 시나리오를 만든 구조적 원인.

- **결정**:
  - **§2.1 Invariant**: 감사 로그는 cross-cutting 관심사이며 도메인 임계 경로 응답을 절대 깨뜨리지 않는다. 도메인 라우트는 `safeAudit(entry, context?)` (`src/lib/audit-log-db.ts`) 만 사용. `writeAuditLogDb` 는 `@internal`.
  - **§2.2 빌드 게이트 + Self-heal 2단계**: `wsl-build-deploy.sh` `[6/8] run-migrations.cjs` + `[7/8] verify-schema.cjs` 가 1차 fail-fast 게이트 (verify 실패 시 PM2 reload 차단). `instrumentation.ts` `applyPendingMigrations()` 가 부팅 2차 자가치유 (warn-only, 부팅 차단 안 함). `pack-standalone.sh` 가 `src/lib/db/migrations/` → `<bundle>/db-migrations/` 동봉.
  - **§2.3 패턴 sweep**: 11개 도메인 콜사이트 (`login-finalizer` / `cleanup-scheduler` / `auth/{logout,refresh,sessions/[id],sessions/revoke-all}` / `admin/users/[id]/sessions` / `tables/[table]{,/[pk],/composite}`) 모두 `safeAudit` 으로 일괄 변환. 정식 파일 §6 근거에 변경 파일 전수.
  - **§2.4 SQLite vs Postgres 통합 보류**: 하이브리드 구조의 의도(감사 트래픽 격리, 백업 분리)가 ADR-013/015 운영 노트에 명시. 통합 트리거 발동 시 별도 ADR.

- **고려한 대안**: A1) 운영 DB 1회 fix 단독 — L2/L3/L4 잔존 / A2) `writeAuditLogDb` 내부 try/catch — 테스트 throw 검증 차단 / A3) outbox 큐 비동기 — 본 사고 직접 원인 아님 / A4) audit_logs 즉시 Postgres 통합 — 백업 정책 재설계 미완 (§2.4 보류). 정식 파일 §3.

- **결과**:
  - **긍정**: 도메인 임계 경로가 audit DB 결함과 분리 — observability 가 도메인을 인질로 잡지 않음. 빈 DB traffic 수락 시나리오 구조적 제거. 패턴 sweep 으로 동일 함정 재발 방지.
  - **부정**: `safeAudit` 실패는 warn 로그만 — 차후 audit-failure 카운터 메트릭 + 알림 별도 트랙 필요. instrumentation self-heal SQLite 잠금 가능성 (WAL 모드라 확률 낮음).

- **검증 결과 (세션 56)**: tsc 0 / vitest 268 PASS (전수, 회귀 0) / 운영 DB 4096 → 36864 bytes 마이그레이션 성공 / 운영 login API 401 in 232ms (이전 throw 500 → fail-soft 401 복구) / PM2 에러 로그 `[cleanup-scheduler] audit log write failed` 5일 연속 발생(2026-04-20~25)이 세션 54 진단 패치 ROI 직접 입증 → 세션 56 로그인 장애로 표면화 → 4층 일괄 해결.

- **근거 문서**:
  - 정식 ADR: `docs/research/decisions/ADR-021-audit-cross-cutting-fail-soft.md`
  - 솔루션 노트: `docs/solutions/2026-04-25-audit-fail-soft-and-migration-self-heal.md`
  - 변경 파일: `src/lib/audit-log-db.ts`, `src/lib/sessions/{cleanup,login-finalizer}.ts`, `src/lib/cleanup-scheduler.ts`, `src/lib/db/migrate.ts` (신규), `src/instrumentation.ts`, 8 도메인 라우트, `scripts/{run-migrations,verify-schema}.cjs` (신규), `scripts/{pack-standalone,wsl-build-deploy}.sh`
  - 직전 진단 패턴: `docs/handover/260425-session54-audit-silent-failure-diagnostic.md` + `docs/solutions/2026-04-25-cleanup-scheduler-audit-silent-failure.md`

- **재검토 트리거** (정식 파일 §5):
  1. SQLite WAL 락 / busy_timeout 초과로 audit 실패율 > 0.1%/일 (3일 연속) → audit 통합 ADR 신설
  2. 감사 보관 3개월 초과 → 단일 SQLite 1GB+ → 운영 부담 가시화
  3. Postgres 트랜잭션 안에서 audit + 도메인 쓰기 합류 신규 요구 (컴플라이언스)
  4. `safeAudit` 호출자 100건 초과 → 메트릭/샘플링 정책 필요
  5. 신규 storage 백엔드(Loki 등) 도입 시 cross-cutting 추상화 재설계
...
│       │ 빌드 게이트 + 마이그레이션 동봉으로 강화            │
│       ▼                                                   │
│ ADR-021 audit fail-soft + migration self-heal (세션 56)    │
│       └─ §2.3 sweep ─── 모든 도메인 라우트가 safeAudit 만 사용 │
│                                                          │
...
1. 이 파일 `01-adr-log.md`에 **다음 ADR 번호를 할당** (현재 **026부터** — ADR-019 세션 30, ADR-020 세션 50, ADR-021 세션 56 등록 / ADR-022~025는 §5 예상 후보로 reserved, 2026-04-25 ADR-021 본 등록 반영)
...
| DB Ops | ADR-005 |
| Auth Core | ADR-006, ADR-017, ADR-019 |
...
| UX Quality | ADR-014 |
| Operations | ADR-015 (Capistrano 부분 ADR-020에 의해 부분 대체), ADR-020 (standalone+rsync 활성), ADR-021 (audit fail-soft + migration self-heal) |
| 전 카테고리 | ADR-001 (Multi-tenancy), ADR-018 (레이어 구조) |

### 4.2 상태별 분포

- **Accepted**: 21건 (100%) — ADR-019 (argon2id, 세션 30) / ADR-020 (standalone, 세션 50) / ADR-021 (audit fail-soft, 세션 56) 추가
- **Proposed**: 0건
- **Deprecated**: 0건
- **Superseded**: 0건 (ADR-015는 Capistrano 부분만 ADR-020에 의해 부분 대체, 나머지 결정은 유효)
...
| 16 (Observability + Operations) | ADR-013, ADR-015, ADR-020 (16 부분 대체), ADR-021 (audit fail-soft) |
...
19 ADR × 평균 2.5 트리거 = **약 48개 재검토 조건** 등록. 이 조건들이 충족되면 ADR 재평가 착수.

(세션 30 SP-010~016 결과 반영으로 다음 재검토 트리거가 **해소**됨:
- ADR-009 재검토 트리거 1 "isolated-vm v6 Node 24 ABI 호환 깨짐" → **RESOLVED** (SP-012)
)
...
- ~~**ADR-019 (예상)**: argon2 전환 시점~~ — **확정되어 본 문서 §1에 ADR-019로 등록됨 (세션 30 SP-011)**. 이하 예상 번호는 한 칸씩 내려감.
- ~~**ADR-020 (예상)**: Prisma 7 → Prisma 8 업그레이드 타이밍~~ — **ADR-020 슬롯은 세션 50 standalone+rsync+pm2 reload에 점유 (2026-04-25 등록)**. Prisma 8 업그레이드 후보는 **ADR-022** 슬롯으로 재할당.
- ~~**ADR-021 (예상)**: 마이그레이션 롤백 5초 구현 패턴~~ — **ADR-021 슬롯은 세션 56 audit cross-cutting fail-soft + 마이그레이션 self-heal 에 점유 (2026-04-25 등록)**. "마이그레이션 롤백 5초"는 ADR-020 standalone+rsync 의 `~/ypserver.previous` 백업 swap (~3-5초 롤백) 으로 사실상 충족 — 별도 ADR 미발행. 5초 이내 보장이 운영상 문제가 되면 ADR-026 후보 슬롯으로 신설.
- **ADR-022 (예상)**: Prisma 7 → Prisma 8 업그레이드 타이밍 — ASM-11 검증 결과 기반 *(2026-04-25 재할당, 원래 ADR-020 후보)*
- **ADR-023 (예상)**: Next.js 16 → 17 업그레이드 전략 — 릴리스 시점 기반 *(2026-04-25 재할당, 원래 ADR-022 후보)*
- **ADR-024 (예상)**: Capacitor 모바일 클라이언트 지원 여부 — DQ-14.x 답변 *(2026-04-25 재할당, 원래 ADR-023 후보)*
...
| DQ-1.10 ~ 1.11 (Realtime 백프레셔) | ADR-021 (예상) | Wave 5 스파이크 결과 *(2026-04-25 재할당, 원래 ADR-019)* |
...
| DQ-14.x (Capacitor 모바일) | ADR-022~024 (예상) | Wave 5 결정 *(2026-04-25 재할당, 원래 ADR-020~023)* |
