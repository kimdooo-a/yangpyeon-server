...
1. 이 파일 `01-adr-log.md`에 **다음 ADR 번호를 할당** (현재 **021부터** — ADR-019 세션 30, ADR-020 세션 50에서 등록)
...
- **ADR-020 (예상)**: Prisma 7 → Prisma 8 업그레이드 타이밍 — ASM-11 검증 결과 기반
- **ADR-021 (예상)**: 마이그레이션 롤백 5초 구현 패턴 — Phase 16 Operations 스파이크 결과
- **ADR-022 (예상)**: Next.js 16 → 17 업그레이드 전략 — 릴리스 시점 기반
