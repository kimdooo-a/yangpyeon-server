...
- ~~**cron / webhooks / api-keys / log-drains / edge_functions / edge_function_runs / webauthn_authenticators / mfa_enrollments** 응답의 ORM-read Date 직렬화 경로~~ ✅ **세션 44 완결** (12 파일 헬퍼 적용).
- ~~**공용 헬퍼 `fetchDateFieldsText(table, ids)`** 도입 검토~~ ✅ **세션 44 완결** (`src/lib/date-fields.ts`).

## 세션 44 추가 — 0-row 테이블 17 파일 Date 직렬화 선제 일괄 수정 + 공용 헬퍼 도입

세션 43의 후속 이월 §2 (0-row 테이블 17 파일) + 공용 헬퍼 권장을 일괄 처리. 데이터 유입 전 선제적으로 패턴을 확산하여 회귀 위험을 영구 차단.

### 헬퍼 설계 — `src/lib/date-fields.ts`

```ts
const ALLOWED_TABLES = new Set<string>([
  "users", "cron_jobs", "webhooks", "api_keys", "log_drains",
  "edge_functions", "edge_function_runs", "mfa_enrollments",
  "mfa_recovery_codes", "webauthn_authenticators", "sessions",
]);
...
export async function fetchDateFieldsText<F extends string>(
  ...
  ids: readonly string[],
  fields: readonly F[],
): Promise<Map<string, Record<F, string | null>>> {
  // ... 화이트리스트 두 겹 검증 후 $queryRaw + Prisma.raw + ::text 캐스팅
}
...
**보안 디자인**:
- `Prisma.raw(table)` / `Prisma.raw(selectClause)` — 동적 식별자 보간 → SQL injection 벡터
- 두 겹 방어: ALLOWED_TABLES Set 강제 + COLUMN_RE 정규식 (소문자 snake_case)
- ids 파라미터는 `${idArray}::text[]` 로 안전 바인딩

**단위 테스트** (`date-fields.test.ts`): 10건
- 빈 ids/fields → 빈 Map (DB 호출 0)
- 화이트리스트 외 테이블 → throw
- SQL injection 시도 → throw (`created_at; DROP TABLE`)
- 대문자 컬럼 → throw (`createdAt`)
- 정상 호출 → Map 반환 + null 처리
- toIsoOrNull null/undefined/empty/timestamptz 4 케이스

### 12 파일 수정 표

| # | 파일 | 핸들러 | 적용 |
|---|------|--------|------|
| 1 | `api/v1/cron/route.ts` | GET, POST | `attachCronDates` 헬퍼 함수 |
| 2 | `api/v1/cron/[id]/route.ts` | GET, PATCH | `withCronDates` 단건 |
| 3 | `api/v1/webhooks/route.ts` | GET, POST | `attachWebhookDates` |
| 4 | `api/v1/webhooks/[id]/route.ts` | GET, PATCH | `withWebhookDates` |
| 5 | `api/v1/api-keys/route.ts` | GET, POST | `attachApiKeyDates` (issueApiKey 결과 attach) |
| 6 | `api/v1/api-keys/[id]/route.ts` | DELETE (revokedAt) | 인라인 헬퍼 호출 |
| 7 | `api/v1/log-drains/route.ts` | GET, POST | `attachLogDrainDates` |
| 8 | `api/v1/log-drains/[id]/route.ts` | GET, PATCH | `withLogDrainDates` |
| 9 | `api/v1/functions/route.ts` | GET (nested runs) | fnDateMap + runDateMap 병행 |
| 10 | `api/v1/functions/[id]/route.ts` | GET, PATCH | `withFunctionDates` |
| 11 | `api/v1/functions/[id]/runs/route.ts` | GET (runs.map) | dateMap.get + spread |
| 12 | `api/v1/auth/mfa/status/route.ts` | GET | enrollment + passkeys 2 헬퍼 호출 (enrollment.id select 추가) |

### 수정 불필요 5 파일

Prisma write-only 후 응답에 Date 미포함 (parsing-side 시프트는 read-back 경계에서만 발생):
- `api/v1/cron/[id]/run/route.ts` — runNow 결과만 반환
- `api/v1/webhooks/[id]/trigger/route.ts` — deliver 결과만 반환
- `api/v1/log-drains/[id]/test/route.ts` — deliver 결과만 반환
- `api/v1/functions/[id]/run/route.ts` — 실행 결과만 반환
- `api/v1/auth/mfa/webauthn/authenticators/[id]/route.ts` — DELETE 만, success boolean

### E2E 검증 (`scripts/session44-verify.sh`, 신규 지속 자산)

```
=== POST /api/v1/webhooks ===
  resp=2026-04-19T01:25:58.098Z pg=2026-04-19T01:25:58.098Z diff_ms=0  ✓
=== GET /api/v1/webhooks/{id} ===
  resp=2026-04-19T01:25:58.098Z pg=2026-04-19T01:25:58.098Z diff_ms=0  ✓
=== GET /api/v1/webhooks (목록) ===
  resp=2026-04-19T01:25:58.098Z pg=2026-04-19T01:25:58.098Z diff_ms=0  ✓
=== POST /api/v1/cron ===
  resp=2026-04-19T01:25:58.461Z pg=2026-04-19T01:25:58.461Z diff_ms=0  ✓
=== POST /api/v1/log-drains ===
  resp=2026-04-19T01:25:58.605Z pg=2026-04-19T01:25:58.605Z diff_ms=0  ✓
```

5건 모두 응답 createdAt === PG `to_char(... AT TIME ZONE 'UTC' ...)` 정확 일치. 9h → 0 수렴.

### 교훈 (세션 44)

- **선제 vs 데이터 유입 시 균형의 재평가**: 세션 43은 "데이터 0건 → 가시 영향 0 → 후속 이월" 결정. 세션 44는 헬퍼 도입으로 12 파일 LOC 비용이 합리적이라 판단 → 선제 일괄 수정 채택. 회귀 가드 자산 1건 + 헬퍼 단일 점검점 확보로 향후 비용 더 낮춤.
- **헬퍼 vs 인라인의 분기점**: 동일 패턴이 5+ 파일에 반복되면 헬퍼 도입 가치가 인라인 LOC 절감을 넘어섬 (단일 점검점 + 보안 일관성 + 향후 endpoint 즉시 적용). 본 세션 12 파일이 임계점 통과.
- **회귀 가드 자산 3종 누적**: session43-parsing-repro.ts (시프트 재현) + session43-verify.sh (users) + session44-verify.sh (webhooks/cron/log-drains). 향후 Prisma 7 패치 / adapter 교체 / 새 테이블 추가 시 30초 실행으로 회귀 즉시 탐지.
